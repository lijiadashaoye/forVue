<template>
  <div class="componentWaper" style="overflow:auto;padding-bottom:20px;">
    <div id="forHeader">
      <p class="isPageName">
    ...
    <el-form v-if="detailType" label-suffix=":" labelWidth="120px" label-position="right">
      <el-form-item label="广告图标题">{{ruleForm.bannerTitle}}</el-form-item>
      <el-form-item label="高亮字体">{{ruleForm.highlight}}</el-form-item>
      <el-form-item label="高亮字体颜色">{{ruleForm.highlight}}</el-form-item>
      <el-form-item label="显示系统">{{ruleForm.sysType}}</el-form-item>
      ...
      <el-form-item label="弹窗文案">{{ruleForm.copyWrit}}</el-form-item>
    ...
    >
      ...
      <el-form-item label="高亮字体颜色" prop="highlightColor">
        <el-input maxlength="30" v-model="ruleForm.highlightColor" placeholder="请输入高亮字体颜色"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    typeChange(value, index) {
      this.ruleForm.bannerButtons[index]["producInfos"]["productInfo"][
        "defaultType"
      ] = false;
      // 外部链接
    ...
    getproList(index) {
      this.getProductList(
        ...
        if (data) {
          if (data.list) {
            this.ruleForm.bannerButtons[index]["producInfos"]["productArr"] =
              data.list;
          }
          if (data.pages) {
            // 获取总页数
