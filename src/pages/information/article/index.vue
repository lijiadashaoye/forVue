  ...
  <div class="componentWaper">
    ...
    >
      ...
      >
        ...
        <el-form-item label="点赞数量" prop="pariseNum">
          <el-input v-model="ruleForm.pariseNum" placeholder="请输入纯数字"></el-input>
        </el-form-item>
        ...
        <el-form-item
          label="封面图片"
          v-if="ruleForm.coverType == 'single_image' || ruleForm.coverType == 'big_image'"
          prop="coverImage"
        >
          <el-upload
        ...
        <el-form-item
          ...
          v-if="linkIdFlag"
          prop="linkId"
          :rules="{ required: this.linkIdFlag, message: '请选择产品', trigger: 'blur'}"
        >
          <el-select
            filterable
...
export default {
  ...
  data() {
    var threeImgRuel = (rule, value, callback) => {
      // 判断手机号是否正确
      if (value.length < 3) {
    ...
    return {
      ...
      rules: {
        ...
        tagId: [
          { required: false, message: "请选择文章类型", trigger: "blur" }
        ],
        tagName: [
        ...
        articleTime: [
          { required: true, message: "请选择文章时间", trigger: "blur" }
        ],
        sendTime: [
        ...
        newCoverList: [
          { required: true, message: "请上传封面图片", trigger: "blur" },
          { validator: threeImgRuel, trigger: "change" }
        ],
        productType: [
  ...
  methods: {
    ...
    userDo() {
      ...
      if (jurisdiction.indexOf("essay_upd_status") > -1) {
        this.tableInputData.data.custom.push({
          text: "修改审核状态",
          type: "success",
          size: "mini",
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.linkLocationOpt &&
            this.linkLocationOpt.forEach(v => {
          ...
          const obj = {
            ...
            pariseNum: this.ruleForm.pariseNum
          };
          if (!this.updateFlag) {
            //新增
