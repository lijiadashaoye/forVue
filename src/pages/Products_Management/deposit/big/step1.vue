<template>
  <div class="componentWaper">
    <div id="forHeader">
      <p class="isPageName">
        <span :class="env?'lineSpan1':'lineSpan'">|</span>
        位置：{{$store.state.for_layout.titles}}{{pageName}}
      </p>
    </div>
    <div style="overflow:auto;" id="forTable">
      <el-form
        ref="ruleForm"
        size="normal"
        :model="ruleForm"
        label-width="150px"
        label-suffix=":"
        class="isForm"
        :rules="rules"
      >
        <el-form-item prop="institutionId" label="机构名称" style="position:relative" class="is50">
          <el-select
            filterable
            class="isInput"
            placeholder="请选择"
            v-model="ruleForm.institutionId"
            @change="change_xilie(ruleForm.institutionId,true)"
          >
            <el-option
              size="mini"
              v-for="item in dictData.jigou"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
          </el-select>
          <a class="isAClick" @click="toJiGou">无机构？</a>
        </el-form-item>

        <el-form-item label="产品系列" class="is50">
          <el-select
            filterable
            class="isInput"
            clearable
            placeholder="请选择"
            v-model="ruleForm.seriesId"
            @change="setXiLie(ruleForm.seriesId)"
          >
            <el-option
              size="mini"
              v-for="item in xilie"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
          </el-select>
          <a class="isAClick" @click="toXiLie">无产品系列？</a>
        </el-form-item>

        <hr />

        <el-form-item label="产品名称" prop="name" class="is50">
          <el-input class="isInput" clearable placeholder="请输入" v-model="ruleForm.name"></el-input>
        </el-form-item>

        <el-form-item label="产品类别别名" class="is50" prop="typeAlias">
          <el-input class="isInput" clearable placeholder="请输入" v-model="ruleForm.typeAlias"></el-input>
        </el-form-item>

        <el-form-item label="期限别名" class="is50" prop="deadlineAlias">
          <el-input class="isInput" clearable placeholder="请输入" v-model="ruleForm.deadlineAlias"></el-input>
        </el-form-item>

        <el-form-item label="利率别名" class="is50" prop="interestRateAlias">
          <el-input
            class="isInput"
            clearable
            placeholder="请输入"
            v-model="ruleForm.interestRateAlias"
          ></el-input>
        </el-form-item>

        <el-form-item label="计息方式" class="is50" prop="jixi">
          <el-select filterable clearable placeholder="请选择" v-model="ruleForm.jixi">
            <el-option
              size="mini"
              v-for="item in dictData.interest_mode"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="付息频率" style="position:relative" class="is50">
          <el-select
            filterable
            class="isInput"
            clearable
            placeholder="请选择"
            v-model="ruleForm.frequencyType"
          >
            <el-option
              size="mini"
              v-for="item in dictData.frequency_type"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="所属区域" style="position:relative" class="is50" prop="areaCode">
          <el-select
            filterable
            class="isInput"
            clearable
            placeholder="请选择"
            v-model="ruleForm.areaCode"
          >
            <el-option
              size="mini"
              v-for="item in dictData.quyu"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="监管属性" style="position:relative" class="is50" prop="regulatoryProperty">
          <el-select
            filterable
            class="isInput"
            clearable
            placeholder="请选择"
            v-model="ruleForm.regulatoryProperty"
          >
            <el-option
              size="mini"
              v-for="item in dictData.regulatory_property"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="产品标签" class="is50">
          <el-select
            filterable
            class="isInput"
            v-model="ruleForm.productTags"
            clearable
            placeholder="请选择"
            multiple
          >
            <el-option
              size="mini"
              v-for="item in dictData.productTags"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
          </el-select>
          <span class="isA">
            <i
              title="标签：标签默认在首页排行榜单显示，活期存款展示前两个标签，智能存款展示前四个标签"
              class="myIcon14px icon-wenhaoyuanyiwenxianxing"
            ></i>
          </span>
        </el-form-item>

        <el-form-item label="自定义标签" class="is50">
          <el-select
            filterable
            class="isInput"
            v-model="ruleForm.selfDefiningTags"
            clearable
            placeholder="请选择"
            multiple
          >
            <el-option
              size="mini"
              v-for="item in dictData.selfDefiningTags"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="活动标签" class="is50">
          <el-select
            filterable
            class="isInput"
            v-model="ruleForm.activityTags"
            clearable
            placeholder="请选择"
            multiple
          >
            <el-option
              size="mini"
              v-for="item in dictData.activityTags"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="产品版本标识" prop="contentVersion" class="is50">
          <el-input
            class="isInput"
            clearable
            v-model="ruleForm.contentVersion"
            placeholder="请输入"
            type="number"
          ></el-input>
        </el-form-item>

        <el-form-item label="币种" style="position:relative" class="is50" prop="currencyCode">
          <el-select
            filterable
            class="isInput"
            clearable
            placeholder="请选择"
            v-model="ruleForm.currencyCode"
          >
            <el-option
              size="mini"
              v-for="item in dictData.bizhong"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="支取时间" class="is50" prop="payTime">
          <el-select
            filterable
            class="isInput"
            v-model="ruleForm.payTime"
            clearable
            placeholder="请选择"
            @change="add_lockDay"
          >
            <el-option
              size="mini"
              v-for="item in dictData.pay_time"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="年收费率" class="is50" prop="yearRate">
          <el-input
            type="number"
            clearable
            v-model="ruleForm.yearRate"
            placeholder="请输入"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="锁定期" class="is50" prop="lockDay" v-if="rules['lockDay']">
          <el-input
            type="number"
            clearable
            v-model="ruleForm.lockDay"
            placeholder="请输入"
            class="isInput"
          ></el-input>
          <span class="isA">天</span>
        </el-form-item>

        <el-form-item label="递增金额" prop="increaseAmount" class="is50">
          <el-input
            class="isInput"
            clearable
            v-model="ruleForm.increaseAmount"
            placeholder="请输入"
            type="number"
          ></el-input>
        </el-form-item>

        <el-form-item label="发行开始时间" class="is50" prop="faxing_start">
          <el-date-picker
            type="datetime"
            value-format="yyyy-MM-dd HH:mm:ss"
            class="isInput"
            v-model="ruleForm.faxing_start"
            placeholder="选择日期"
          ></el-date-picker>
        </el-form-item>

        <el-form-item label="发行结束时间" class="is50" prop="faxing_end">
          <el-date-picker
            type="datetime"
            value-format="yyyy-MM-dd HH:mm:ss"
            class="isInput"
            v-model="ruleForm.faxing_end"
            placeholder="选择日期"
          ></el-date-picker>
        </el-form-item>

        <el-form-item label="发行量" prop="faxing_num" class="is50">
          <el-input
            class="isInput"
            clearable
            v-model="ruleForm.faxing_num"
            placeholder="请输入"
            type="number"
          ></el-input>
        </el-form-item>

        <el-form-item label="利息计算天数" prop="jisuan_day" class="is50">
          <el-input
            class="isInput"
            clearable
            v-model="ruleForm.jisuan_day"
            placeholder="请输入"
            type="number"
          ></el-input>
        </el-form-item>

        <el-form-item label="买方折算年化利率" style="width:100%;" prop="zhesuan">
          <el-input
            clearable
            v-model="ruleForm.zhesuan"
            placeholder="请输入数字，并以“,”逗号分隔"
            type="textarea"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="存入规则" style="width:100%;" prop="cunru">
          <el-input
            clearable
            v-model="ruleForm.cunru"
            placeholder="请输入"
            type="textarea"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="转让规则" style="width:100%;" prop="zhuanrang">
          <el-input
            clearable
            v-model="ruleForm.zhuanrang"
            placeholder="请输入"
            type="textarea"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="转出规则" style="width:100%;" prop="zhuanchu">
          <el-input
            clearable
            v-model="ruleForm.zhuanchu"
            placeholder="请输入"
            type="textarea"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="支取规则" style="width:100%;" prop="zhiqu">
          <el-input
            clearable
            v-model="ruleForm.zhiqu"
            placeholder="请输入"
            type="textarea"
            class="isInput"
          ></el-input>
        </el-form-item>

        <el-form-item label="最大起购金额(元)" prop="maxAmount" class="is50">
          <el-input
            type="number"
            class="isInput"
            clearable
            v-model="ruleForm.maxAmount"
            placeholder="请输入"
          ></el-input>
        </el-form-item>

        <el-form-item label="最小起购金额(元)" prop="minAmount" class="is50">
          <el-input
            type="number"
            class="isInput"
            clearable
            v-model="ruleForm.minAmount"
            placeholder="请输入"
          ></el-input>
        </el-form-item>

        <el-form-item label="起购范围说明" style="text-align:left;width:100%;margin-right:30px;">
          <editor-bar v-model="ruleForm.amountRangeExplain" :url="'admin/file/up/product'"></editor-bar>
        </el-form-item>

        <el-form-item
          prop="description"
          label="产品描述"
          style="text-align:left;width:100%;margin-right:30px;"
        >
          <editor-bar v-model="ruleForm.description" :url="'admin/file/up/product'"></editor-bar>
        </el-form-item>
      </el-form>
      <div style="width:90%;margin:0 auto;padding-left:50px;">
        <addLiLv :addLiLvProp="addLiLvProp"></addLiLv>
      </div>

      <div class="nextButtons">
        <el-button size="mini" type="primary" @click="next">下一步</el-button>&nbsp;&nbsp;&nbsp;&nbsp;
        <el-button size="mini" type="info" @click="back">取消</el-button>
      </div>
    </div>
  </div>
</template>
<script>
import EditorBar from "@/components/wangEnduit";
import addLiLv from "@/components/addLiLv.vue";

export default {
  components: { EditorBar, addLiLv },
  data() {
    // 验证数字
    var checkNum2 = (rule, value, callback) => {
      if (+value < 0 || +value > 100) {
        callback(new Error("请输入0--100"));
      } else if (("" + value).length > 10 || ("" + value).length < 0) {
        callback(new Error("请输入1-10字符"));
      } else {
        callback();
      }
    };
    // 验证数字
    var checkNum3 = (rule, value, callback) => {
      if (+value < 0 || +value >= 10000000 || !value) {
        callback(new Error("请输入0--10000000正数"));
      } else {
        this.ruleForm.increaseAmount = (+value).toFixed(2);
        callback();
      }
    };

    // 验证数字
    var checkNum4 = (rule, value, callback) => {
      if (+value < 0 || !value || +value > 10000000) {
        callback(new Error("请输入0--10000000正数"));
      } else if (+value < +this.ruleForm.minAmount) {
        callback(new Error("不能小于最小起购额"));
      } else {
        this.ruleForm.maxAmount = (+value).toFixed(2);
        callback();
      }
    };
    // 验证数字
    var checkNum5 = (rule, value, callback) => {
      if (+value < 0 || !value || +value > 10000000) {
        callback(new Error("请输入0--10000000正数"));
      } else if (+value > +this.ruleForm.maxAmount) {
        callback(new Error("不能大于最大起购额"));
      } else {
        this.ruleForm.minAmount = (+value).toFixed(2);
        callback();
      }
    };
    // 验证数字
    var check_faxing_num = (rule, value, callback) => {
      if (+value < 0 || +value >= 10000000) {
        callback(new Error("请输入0--10000000正数"));
      } else {
        this.ruleForm.faxing_num = (+value).toFixed(0);
        callback();
      }
    };

    var check_faxing_start = (rule, value, callback) => {
      if (!value) {
        callback();
      } else if (!value && !this.ruleForm.faxing_end) {
        callback(new Error("请同时输入发行开始时间和结束时间"));
      } else if (new Date(value) > new Date(this.ruleForm.faxing_end)) {
        callback(new Error("发行开始时间不能晚于结束时间"));
      } else {
        callback();
      }
    };
    var check_faxing_end = (rule, value, callback) => {
      if (!value) {
        callback();
      } else if (!value && !this.ruleForm.faxing_start) {
        callback(new Error("请同时输入发行开始时间和结束时间"));
      } else if (new Date(value) < new Date(this.ruleForm.faxing_start)) {
        callback(new Error("发行结束时间不能早于开始时间"));
      } else {
        callback();
      }
    };

    // 验证数字
    var check_jisuan_day = (rule, value, callback) => {
      if (+value < 0 || +value >= 10000000) {
        callback(new Error("请输入0--10000000正数"));
      } else {
        this.ruleForm.jisuan_day = (+value).toFixed(0);
        callback();
      }
    };

    return {
      env: null,
      addLiLvProp: {}, // 传给利率表单的数据
      xilie: [], // 从服务器返回的产品系列数据
      pageName: "", // 当前页面名字
      dictData: {}, // 字典数据
      cunqi_symbol: {
        forMin: [], // 最小期限限制条件
        forMax: [] // 最大期限限制条件
      },
      ruleForm: {
        deadlineAlias: "", // 期限别名
        institutionId: "", // 机构名称
        seriesId: "", // 产品系列ID
        name: "", // 产品名称
        interestRateAlias: "", // 利率别名
        typeAlias: "", // 产品类型（类别别名）
        cunru: "", // 存入规则
        frequencyType: "", // 付息频率
        yearRate: "", //  年收费率
        maxAmount: "", // 默认最大存款金额
        minAmount: "", // 默认最小存款金额
        amountRangeExplain: "", // 默认存款金额说明
        increaseAmount: "", // 递增金额
        productTags: [], // 产品标签
        selfDefiningTags: [], // 自定义标签
        activityTags: [], // 活动标签
        payTime: "", // 支付时间
        lockDay: "",
        regulatoryProperty: "", // 监管属性
        currencyCode: "CNY", // 币种编码
        contentVersion: "", // 内容版本号
        description: "", // 产品描述
        areaCode: "100000", // 所属区域
        lilv: [],
        faxing_start: "", // 发行开始时间
        faxing_end: "", // 发行结束时间
        faxing_num: "", // 发行量
        jixi: "", // 计息方式
        zhuanrang: "", // 转让规则
        zhuanchu: "", // 转出规则
        zhiqu: "", // 支取规则
        jisuan_day: "", // 利息计算天数
        zhesuan: "" // 买方折算年化利率
      },
      //表单验证
      rules: {
        deadlineAlias: [
          { min: 1, max: 50, message: "最多输入50个字", trigger: "blur" }
        ],
        jisuan_day: [
          { required: true, validator: check_jisuan_day, trigger: "blur" }
        ],
        jixi: [
          { required: true, message: "请选择计息方式", trigger: "change" }
        ],
        areaCode: [
          { required: true, message: "请选择所属区域", trigger: "change" }
        ],

        institutionId: [
          { required: true, message: "请选择机构名称", trigger: "change" }
        ],
        name: [
          { required: true, message: "请输入产品名称", trigger: "blur" },
          { min: 1, max: 50, message: "最多输入50个字", trigger: "blur" }
        ],
        typeAlias: [
          { min: 1, max: 20, message: "最多输入20个字", trigger: "blur" }
        ],
        interestRateAlias: [
          { min: 1, max: 20, message: "最多输入20个字", trigger: "blur" }
        ],
        yearRate: [{ validator: checkNum2, trigger: "blur" }],

        increaseAmount: [
          { required: true, validator: checkNum3, trigger: "blur" }
        ],
        minAmount: [{ required: true, validator: checkNum5, trigger: "blur" }],
        maxAmount: [{ required: true, validator: checkNum4, trigger: "blur" }],

        currencyCode: [
          { required: true, message: "请选择", trigger: "change" }
        ],
        payTime: [{ required: true, message: "请选择", trigger: "change" }],
        regulatoryProperty: [
          { required: true, message: "请选择", trigger: "change" }
        ],
        faxing_start: [{ validator: check_faxing_start, trigger: "blur" }],
        faxing_end: [{ validator: check_faxing_end, trigger: "blur" }],
        faxing_num: [{ validator: check_faxing_num, trigger: "blur" }]
      }
    };
  },
  created() {
    this.env = sessionStorage.getItem("env") === "development";
    this.pageName = sessionStorage.getItem("page") + " > 新增大额存单第一步"; // 获取页面名称
    this.dictData = JSON.parse(sessionStorage.getItem("dict"));
    // 获取选定的机构
    let jigouData = JSON.parse(sessionStorage.getItem("select_xilie"));

    if (sessionStorage.getItem("big_step1")) {
      this.ruleForm = JSON.parse(sessionStorage.getItem("big_step1"));
      this.change_xilie(this.ruleForm.institutionId, false);
    } else {
      this.change_xilie(jigouData.institutionId, false);
      this.ruleForm.institutionId = jigouData.institutionId;
      if (jigouData.id) {
        this.ruleForm.seriesId = jigouData.id;
      }
    }
    this.addLiLvProp = {
      isInput: false, // 记录表单是否发生数据输入
      lilv_data: this.ruleForm.lilv,
      num: this.ruleForm.lilv.length
        ? Math.max(...this.ruleForm.lilv.map(tar => tar.num)) + 1
        : 0
    };
  },

  methods: {
    // 如果选择锁定期后可以支取
    add_lockDay() {
      let ru = { ...this.rules };
      if (this.ruleForm.payTime === "lock-in_period_draw") {
        ru["lockDay"] = [
          { required: true, validator: this.check_lockDay, trigger: "blur" }
        ];
      } else {
        delete ru["lockDay"];
      }
      this.rules = { ...ru };
    },
    check_lockDay(rule, value, callback) {
      if (+value < 0 || !value || +value > 10000000) {
        callback(new Error("请输入0--10000000正数"));
      } else {
        callback();
      }
    },
    // 变更产品系列后，要保存选择的系列，等回到当前页面后数据能对应上
    setXiLie(data) {
      if (typeof data === "number") {
        let tar = this.xilie.filter(item => item.id === data);
        sessionStorage.setItem("select_xilie", JSON.stringify(tar[0]));
      } else {
        let xilieData = JSON.parse(sessionStorage.getItem("select_xilie")),
          obj = {
            institutionId: xilieData.institutionId,
            institutionName: xilieData.institutionName
          };
        sessionStorage.setItem("select_xilie", JSON.stringify(obj));
      }
    },
    // 变更机构后，要对应调整系列
    change_xilie(id, type) {
      let xilieData = JSON.parse(sessionStorage.getItem("xilie_cunkuan"));
      this.xilie = xilieData.filter(
        item => item.institutionId === id
      )[0].seriesList;
      if (this.ruleForm.seriesId != "" && type) {
        this.ruleForm.seriesId = "";
      }
    },
    /*****************************************************************************************************/
    // 下一步
    next() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (this.ruleForm.lilv.length) {
            if (this.addLiLvProp.isInput) {
              this.$confirm(
                `利率表单已经填过数据，但不完整，是否抛弃已填数据从而进入下一步？`
              )
                .then(() => {
                  sessionStorage.setItem(
                    "big_step1",
                    JSON.stringify(this.ruleForm)
                  );
                  this.$router.push({
                    name: "big_step2"
                  });
                })
                .catch(() => {});
            } else {
              sessionStorage.setItem(
                "big_step1",
                JSON.stringify(this.ruleForm)
              );
              this.$router.push({
                name: "big_step2"
              });
            }
          } else {
            this.$message.error("请填写利率数据！");
          }
        } else {
          this.$message.error("请正确填写表单数据！");
        }
      });
    },

    // 取消
    back() {
      this.$router.push({
        name: "deposit_mainPage"
      });
    },
    toJiGou() {
      sessionStorage.setItem("page", "机构管理");
      this.$router.push({
        name: "organizational_step1"
      });
    },
    toXiLie() {
      this.$router.push({
        name: "deposit",
        query: {
          institutionId: this.ruleForm.institutionId
        }
      });
    }
  }
};
</script>