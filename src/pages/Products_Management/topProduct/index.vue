<template>
    <div class="componentWaper">
            ...
            <div class="search">
                <el-form :inline="true" :model="searchForm" :rules="rules" ref="searchForm" class="demo-form-inline">
                    <el-form-item label="币种" prop="currencyCode">
                        <el-select v-model="searchForm.currencyCode" placeholder="请选择" clearable filterable size="mini">
                            <el-option
                    ...
                    <el-form-item label="期限" prop="periodCode">
                        <el-select v-model="searchForm.periodCode" placeholder="请选择" clearable filterable size="mini">
                            <el-option
        ...
        <!-- 产品列表弹框 -->
        <el-dialog title="产品列表" :visible.sync="dialogTableVisible" :close-on-click-modal='false' width="90%">
            <div class="search">
                <el-form :inline="true" :model="proSearch" :rules="proRules" ref="proSearch"  class="demo-form-inline">
                    <el-form-item label="期限" prop='periodCode'>
                        <el-select v-model="proSearch.periodCode" placeholder="请选择" clearable filterable size="mini">
                            <el-option
                                v-for="(item,ind) in periodList"
                                :key="ind"
                                :label="item.peroidName"
                    ...
                    <el-form-item label="币种" prop='currencyCode'>
                        <el-select v-model="proSearch.currencyCode" placeholder="请选择" clearable filterable size="mini">
                            <el-option
                                v-for="(item,ind) in currencyList"
                                :key="ind"
                                :label="item.currencyName"
            ...
            <productTable
                :inputData="this.$store.state.topProduct.pruductList"
                @tableEmit='topEmit'
            />
            <el-dialog title="置顶序号" :visible.sync="topProductVisible" :close-on-click-modal='false' append-to-body>
                <el-form :model="addTop" ref="addTop" class='demo-ruleForm' label-position="left">
                    ...
                    :rules="[
                        ...
                        <el-select v-model="addTop.periodCode" placeholder="请选择" clearable filterable>
                            <el-option
                                v-for="(item,ind) in periodList"
                                :key="ind"
                                :label="item.peroidName"
...
import { currency_Code, period_Code, top_product_delete, top_product_upData, top_product_add } from '../../../api/setting_use';
export default {
    components: {
        isTable,
    ...
    mounted() {
        ...
        currency_Code(this.currencyBigType).then(res=> {//币种列表
            if(res && res.success) {
                this.currencyList = res.data
            }
        });
        period_Code({appChannelCode: 'bicai',peroidType: this.currencyBigType}).then(res=> {//期限列表
    ...
    methods: {
        ...
        handleClick(tab) {
            ...
            this.getList({
                ...
                pageSize: this.$store.state.topProduct.topProductList.pageSize,
                currencyBigType: this.currencyBigType,
                currencyCode: this.searchForm.currencyCode != '' && this.searchForm.currencyCode != null ? this.searchForm.currencyCode : null,
                periodCode: this.searchForm.periodCode != '' && this.searchForm.periodCode != null ? this.searchForm.periodCode : null
            });
            this.getProList({
                pageNum: this.$store.state.topProduct.pruductList.pageNum,
                pageSize: this.$store.state.topProduct.pruductList.pageSize,
        ...
        proSubmit() {
            this.getProList({
                pageNum: this.$store.state.topProduct.pruductList.pageNum,
                pageSize: this.$store.state.topProduct.pruductList.pageSize,
                periodCode: this.proSearch.periodCode != '' && this.proSearch.periodCode ? this.proSearch.periodCode : null,
                currencyCode: this.proSearch.currencyCode != '' && this.proSearch.currencyCode ? this.proSearch.currencyCode : null,
                productName:  this.proSearch.productName != '' && this.proSearch.productName ? this.proSearch.productName : null,
                institutionName:  this.proSearch.institutionName != '' && this.proSearch.institutionName ? this.proSearch.institutionName : null,
                areaCode: this.currencyBigType
            })
        },
        //置顶
